...
local hasMovedFarOnce = false -- للتأكد أنه تحرك بعيداً عن الهدف أولاً
        ...
        if isFollowing and targetName ~= "" and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            if targetPlr and targetPlr.Character and targetPlr.Character:FindFirstChild("HumanoidRootPart") then
                if myChar.Humanoid.Health > 0 then
                    ...
                    -- التأكد أنه ابتعد عن الهدف أولاً (أكثر من 6 أمتار) لكي لا يموت بالخطأ عند التشغيل
                    if distance > 6 then
                        hasMovedFarOnce = true
                    end
                    
                    -- إذا كان قد تحرك بعيداً ثم اقترب مجدداً (أقل من 5 أمتار)، يذبح نفسه
                    if distance <= 5 and hasMovedFarOnce and not hasKilledCurrentLife then
