-- ==========================================================
--          WAFI ULTIMATE HUB + HITBOX EXPANDER [LOW PLAYERS]
-- ==========================================================
...
-- ==========================================================
-- نظام الفحص والبحث التلقائي عن عسكري معاه أكثر من 500k
-- ==========================================================
local AutoHopFileName = "Wafi_AutoCopHop.txt"
local CopTeamNames = {"Police", "Guards", "الشرطة", "عساكر", "العساكر", "شرطة", "Cop", "Cops", "Guard"} 
local MoneyNames = {"cash", "money", "فلوس", "الكاش", "مصاري", "المبلغ", "coins", "wallet"}

local function isCop(player)
    if player and player.Team then
        for _, teamName in ipairs(CopTeamNames) do
            if string.find(string.lower(player.Team.Name), string.lower(teamName)) then
                return true
            end
        end
    end
...
local function getPlayerMoney(player)
    ...
    if stats then
        for _, val in ipairs(stats:GetChildren()) do
            local valName = string.lower(val.Name)
            for _, mName in ipairs(MoneyNames) do
                if string.find(valName, mName) then
                    if val:IsA("IntValue") or val:IsA("NumberValue") or val:IsA("DoubleConstrainedValue") then
                        total = total + val.Value
                    elseif val:IsA("StringValue") then
                        local cleanStr = string.gsub(val.Value, "[,%$]", "")
...
local function hasRichCopsInServer()
...
-- دالة الانتقال لسيرفرات بها أقل عدد من اللاعبين
local function hopToLowPlayerServer()
    if writefile then pcall(function() writefile(AutoHopFileName, "true") end) end
    local placeId = game.PlaceId
    local req = (syn and syn.request) or (http and http.request) or http_request or request or httprequest
    -- استخدام sortOrder=Asc لجلب السيرفرات بأقل عدد لاعبين
    ...
    pcall(function()
        ...
        if res and res.data then
            ...
            for _, server in ipairs(res.data) do
                -- البحث عن سيرفر ليس سيرفرنا الحالي، وفيه خانات فارغة، وعدد أفراده قليل جداً (أقل من 6 لاعبين مثلاً)
            ...
            if #lowPlayerServers > 0 then
                -- اختيار أحد أقل السيرفرات ازدحاماً
                local chosenServer = lowPlayerServers[math.random(1, math.min(#lowPlayerServers, 5))]
...
-- فحص تلقائي فور دخول السيرفر
task.spawn(function()
    if readfile and isfile and isfile(AutoHopFileName) then
        local isSearching = false
        pcall(function() isSearching = (readfile(AutoHopFileName) == "true") end)
        if isSearching then
            task.wait(3.5)
            local found, copPlayer = hasRichCopsInServer()
            if found then
                if writefile then pcall(function() writefile(AutoHopFileName, "false") end) end
                warn("[WAFI HUB] تم العثور على شرطي غني: " .. copPlayer.Name .. " بحوزته أكثر من 500k!")
            else
                warn("[WAFI HUB] لا يوجد شرطي يملك 500k، جاري البحث في سيرفر قليل اللاعبين...")
...
local PlayerBox, FarmDropBtn, FarmScroll = createDropdown(FarmPage, UDim2.new(0, 10, 0, 36), UDim2.new(0, 225, 0, 26), "اسم اللاعب أو اختره... ▼")
local ViewPlayerBox, ViewDropBtn, ViewScroll = createDropdown(ViewPage, UDim2.new(0, 10, 0, 8), UDim2.new(0, 225, 0, 28), "اسم اللاعب أو اختره... ▼")
...
ServerHopBtn.Text = "بحث (سيرفرات فارغة) +500k"
...
ServerHopBtn.MouseButton1Click:Connect(function()
    ...
    if isSearching then
        if writefile then pcall(function() writefile(AutoHopFileName, "false") end) end
        ServerHopBtn.Text = "تم إيقاف البحث التلقائي"
        task.wait(1.5)
        ServerHopBtn.Text = "بحث (سيرفرات فارغة) +500k"
    else
        ServerHopBtn.Text = "جاري فحص فلوس العساكر..."
        local found, copPlayer = hasRichCopsInServer()
        if found then
            ServerHopBtn.Text = "تم العثور على: " .. copPlayer.DisplayName
            task.wait(2)
            ServerHopBtn.Text = "بحث (سيرفرات فارغة) +500k"
        else
            ServerHopBtn.Text = "جاري النقل لسيرفر قليل اللاعبين..."
            hopToLowPlayerServer()
        end
    end
end)
            ...
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character then
                    ...
                    if hrp and humanoid and humanoid.Health > 0 then
                        hrp.Size = HitboxSettings.Size
                        hrp.Transparency = HitboxSettings.Transparency
                        hrp.CanCollide = HitboxSettings.CanCollide
                    end
                end
            end
