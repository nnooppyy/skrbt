-- ==========================================================
--        WAFI ULTIMATE HUB + EXACT SERVER JOIN (FIXED)
-- ==========================================================
...
MiscPage.CanvasSize = UDim2.new(0, 0, 0, 420)
...
Players.PlayerAdded:Connect(function()
    if isOpenFarm then refreshDropdownList(FarmScroll, true) end
    if isOpenView then refreshDropdownList(ViewScroll, false) end
end)
...
-- عناصر MISC المحدثة لدعم السيرفر الدقيق (Job ID)
...
CopyCurrentBtn.Text = "نسخ (الماب + السيرفر) الحالي"
...
DirectIdBox.PlaceholderText = "الصق الـ ID هنا (ماب أو مع السيرفر)..."
DirectIdBox.Text = ""
...
JoinIdBtn.Text = "انتقال إلى السيرفر (نفس السيرفر)"
...
local function refreshSavedUI()
    ...
    for i, data in ipairs(savedPlaceIds) do
        ...
        local displayTxt = tostring(data.placeId)
        if data.jobId and data.jobId ~= "" then
            displayTxt = data.placeId .. " (سيرفر)"
        end
...
local function addSavedID(pId, jId)
    ...
    for _, v in ipairs(savedPlaceIds) do 
        if v.placeId == numId and v.jobId == jId then return end 
    end
    table.insert(savedPlaceIds, {placeId = numId, jobId = jId or ""})
    saveIDsToFile()
    refreshSavedUI()
end
...
-- نسخ الماب مع السيرفر تلقائياً للحافظة
CopyCurrentBtn.MouseButton1Click:Connect(function()
    local currentId = game.PlaceId
    local currentJob = game.JobId
    local combinedStr = currentId .. "," .. currentJob
    copyToClipboard(combinedStr)
    addSavedID(currentId, currentJob)
...
-- زر الانتقال المحدث (يدعم السيرفر الدقيق والماب العادي)
JoinIdBtn.MouseButton1Click:Connect(function()
    local rawText = DirectIdBox.Text
    local pId, jId = parsePlaceAndJob(rawText)
    
    if pId then
        addSavedID(pId, jId)
        JoinIdBtn.Text = "جاري النقل لنفس السيرفر..."
        JoinIdBtn.TextColor3 = Theme.ON
        pcall(function()
            if jId and jId ~= "" then
        ...
        JoinIdBtn.TextColor3 = Color3.fromRGB(220, 50, 50)
        task.wait(1.5)
        JoinIdBtn.Text = "انتقال إلى السيرفر (نفس السيرفر)"
        JoinIdBtn.TextColor3 = Theme.OFF
    end
...
local function toggleFEInvisibility(state)
    ...
    local rootJoint = root:FindFirstChild("RootJoint") or (char:FindFirstChild("LowerTorso") and char.LowerTorso:FindFirstChild("Root"))
    
    if state then
        if rootJoint and not rootJoint:FindFirstChild("Wafi_OrigC0") then
            local val = Instance.new("CFrameValue")
            val.Name = "Wafi_OrigC0"
            val.Value = rootJoint.C0
            val.Parent = rootJoint
        end
        
        if rootJoint then
            rootJoint.C0 = CFrame.new(0, -99999, 0)
        end
        
        if invisRenderConn then invisRenderConn:Disconnect() end
        invisRenderConn = RunService.RenderStepped:Connect(function()
        ...
        if invisRenderConn then
            invisRenderConn:Disconnect()
            invisRenderConn = nil
        end
        
        if rootJoint then
            local orig = rootJoint:FindFirstChild("Wafi_OrigC0")
            if orig then
