...
CopyCurrentBtn.MouseButton1Click:Connect(function()
    ...
    -- دمج رقم الماب مع رقم السيرفر بفاصلة عشان يقدر يقرأه ويدخلك نفس السيرفر
    local fullId = tostring(placeId) .. "," .. tostring(jobId)
    copyToClipboard(fullId)
    
    addSavedID(placeId) -- حفظ رقم الماب فقط في القائمة الجانبية عشان ما يخرب شكلها
...
-- يعمل بدون التواجد تحت الأرض نهائياً!
---------------------------------------------------------
...
local function toggleFEInvisibility(state)
    ...
    if state then
        ...
        -- فصل المظهر الخارجي عن السيرفر دون النزول تحت الأرض
        if rootJoint then
            rootJoint.C0 = CFrame.new(0, -99999, 0)
        end
        
        -- إبقاء الشخصية شفافة محلياً لتستطيع التفاعل والرؤية
