...
-- نظام حفظ وقراءة الـ IDs المحفوظة
local savedPlaceIds = {}
...
-- نسخ للنص إلى المحافظة (Clipboard)
...
InvisBtn.Text = "نمط الاختفاء  [ OFF ]"
...
---------------------------------------------------------
-- عناصر صفحة MISC (ميزات الـ Place ID والسيرفرات)
---------------------------------------------------------
...
DirectIdBox.PlaceholderText = "أدخل Place ID للانتقال..."
DirectIdBox.Text = ""
...
---------------------------------------------------------
-- تحديث واجهة قائمة الـ IDs المحفوظة
---------------------------------------------------------
...
JoinIdBtn.MouseButton1Click:Connect(function()
    local inputId = tonumber(DirectIdBox.Text)
    if inputId then
        addSavedID(inputId)
        JoinIdBtn.Text = "جاري النقل..."
        TeleportService:Teleport(inputId, LocalPlayer)
    else
...
RejoinBtn.MouseButton1Click:Connect(function()
    RejoinBtn.Text = "جاري إعادة الاتصال بنفس السيرفر..."
    ...
    if not success then
        RejoinBtn.Text = "فشل! إعادة المحاولة..."
        task.wait(1)
        TeleportService:Teleport(pId, LocalPlayer)
    end
end)
...
---------------------------------------------------------
-- البحث عن سيرفر قليل عشوائي والدخول إليه
---------------------------------------------------------

ServerHopBtn.MouseButton1Click:Connect(function()
    ServerHopBtn.Text = "جاري البحث عن سيرفر قليل..."
    ...
    task.spawn(function()
        ...
        if success and result and result.data then
            ...
            if #candidateServers > 0 then
                ...
                local poolSize = math.min(#candidateServers, 5)
                local randomIndex = math.random(1, poolSize)
                local chosenServer = candidateServers[randomIndex]

                ServerHopBtn.Text = "تم العثور! جاري النقل..."
                TeleportService:TeleportToPlaceInstance(placeId, chosenServer.id, LocalPlayer)
            else
                ServerHopBtn.Text = "لم يتم العثور على سيرفر قليل!"
                task.wait(1.5)
                ServerHopBtn.Text = "دخول سيرفر قليل (عشوائي)"
            end
        else
            ServerHopBtn.Text = "فشل الاتصال بالخادم!"
            task.wait(1.5)
            ServerHopBtn.Text = "دخول سيرفر قليل (عشوائي)"
        end
    end)
...
-- مؤقت القفز للسكين (كل 5 دقائق)
local lastKnifeJumpTick = os.clock()

-- دالة تنفيذ القفز المضمون
...
-- دالة الاختفاء الحقيقي (FE Invisibility + الشفافية لك 0.5)
local invisLoopRunning = false
local function applyInvisibility(state)
    local char = LocalPlayer.Character
    if not char then return end
    
    -- 1. الشفافية محلياً لك فقط (0.5)
    pcall(function()
    ...
    -- 2. قطع الاتصال البصري عن السيرفر (اخفاء حقيقي عن اللاعبين الآخرين)
    if state then
        if not invisLoopRunning then
            invisLoopRunning = true
            task.spawn(function()
                while isInvisMode and LocalPlayer.Character == char do
                    pcall(function()
                        local root = char:FindFirstChild("HumanoidRootPart")
                        if root then
                            local currentCF = root.CFrame
                            root.CFrame = currentCF * CFrame.new(0, -500, 0)
                            task.wait()
            ...
            if myChar and targetPlr and targetPlr.Character then
                ...
                if myHum and myRoot and tHum and tRoot and myHum.Health > 0 and tHum.Health > 0 then
                    ...
                    elseif isKillMode then
                        -- فحص وتأكيد القفز كل 300 ثانية (5 دقائق)
                        ...
                        -- الهجوم من مسافة 4
                        if dist <= 4 then
                            -- توجيه الشاشة والشخصية لوجه الهدف مباشرة
