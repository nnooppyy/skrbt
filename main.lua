...
---------------------------------------------------------
-- محرك الاختفاء المحسن (Heartbeat Frame-Desync Invisibility)
---------------------------------------------------------
local invisHeartbeat = nil
local invisRender = nil
...
local function toggleFEInvisibility(state)
    ...
    if state then
        -- 1. تجعل الشخصية شبه شفافة (0.5) على شاشتك فقط لترى تحركاتك
        pcall(function()
        ...
        if invisRender then invisRender:Disconnect() end

        local lastSavedCFrame = root.CFrame

        -- 2. حزمة الفيزياء المتجهة للسيرفر (تحت الأرض بـ 300 ستد في لحظة معالجة الفيزياء)
        invisHeartbeat = RunService.Heartbeat:Connect(function()
            if isInvisMode and LocalPlayer.Character == char and root then
                lastSavedCFrame = root.CFrame
                root.CFrame = lastSavedCFrame * CFrame.new(0, -300, 0)
            end
        end)

        -- 3. حزمة العرض المحلي (تضمن بقاءك بنفس المكان بسلاسة تامة على شاشتك وبدون تعليق)
        invisRender = RunService.RenderStepped:Connect(function()
            if isInvisMode and LocalPlayer.Character == char and root then
                root.CFrame = lastSavedCFrame
            end
        end)
    else
        if invisHeartbeat then invisHeartbeat:Disconnect() invisHeartbeat = nil end
        if invisRender then invisRender:Disconnect() invisRender = nil end
