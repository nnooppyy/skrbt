...
-- 4. زر الذهاب والتتبع المستمر (اللوب اللانهائي)
...
-- الحلقة اللانهائية (Loop): يروح للاعب -> يذبح نفسه -> يرسبن -> يرجع يعيد
task.spawn(function()
    while task.wait(0.1) do
        ...
        if myChar and myChar:FindFirstChild("Humanoid") and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            -- يخلي سرعتك شغالة دايم حتى لو رسبنت
            hum.WalkSpeed = currentSpeed
            
            -- إذا الخيار ON
            if isFollowing and targetName ~= "" then
                -- يتأكد إنك حي عشان يمشيك
                if hum.Health > 0 then
                    local targetPlr = findPlayer(targetName)
                    
                    -- يتأكد إن الهدف موجود وحي
                    if targetPlr and targetPlr.Character and targetPlr.Character:FindFirstChild("HumanoidRootPart") and targetPlr.Character:FindFirstChild("Humanoid") then
                        local targetRoot = targetPlr.Character.HumanoidRootPart
                        local targetHum = targetPlr.Character.Humanoid
                        
                        if targetHum.Health > 0 then
                            local distance = (targetRoot.Position - myRoot.Position).Magnitude
                            
                            -- إذا وصل عنده (أقرب من 5 خطوات) يذبح نفسه
                            if distance <= 5 then
                                hum.Health = 0
                            else
                                -- إذا لسه بعيد، يستمر يركض وراه
