...
local targetName = ""
local spawnProtected = false -- متغير الحماية الجديد
...
-- نظام حماية الرسبن (السر هنا)
LocalPlayer.CharacterAdded:Connect(function(char)
    spawnProtected = true -- أول ما ترسبن يفعل الحماية
    task.wait(1.5) -- يعطيك ثانية ونص عشان تتحدث إحداثياتك باللعبة وتبدأ تركض
    
    if char:FindFirstChild("Humanoid") then
        char.Humanoid.WalkSpeed = currentSpeed
    end
    
    spawnProtected = false -- يفك الحماية عشان تقدر تذبح نفسك إذا وصلت
end)
        ...
        if isFollowing and targetName ~= "" then
            ...
            if myChar and targetPlr and targetPlr.Character then
                ...
                if myHum and myRoot and tHum and tRoot and myHum.Health > 0 and tHum.Health > 0 then
                    ...
                    -- هنا الشرط: إذا وصلت له + أنت مو محمي بسبب الرسبن = يذبح نفسه
                    if dist.Magnitude <= 3.5 and not spawnProtected then
                        myHum.Health = 0
                    else
                        -- يركض له
