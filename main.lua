...
-- إيقاف أنيميشن المشي لجعل الشخصية تنزلق وهي واقفة ثابتة
local function freezeAnimations(freeze)
    ...
    if myChar then
        ...
        if myHum then
            ...
            if origHipHeight then
                myHum.HipHeight = origHipHeight
            else
                myHum.HipHeight = (myHum.RigType == Enum.HumanoidRigType.R6 and 0 or 2)
            end
        end
    end
...
GoBtn.MouseButton1Click:Connect(function()
    ...
    if isSuicideMode then
        ...
        if targetPlr then
            ...
            -- تطبيق الارتفاع البسيط + تجميد المشي
            local myChar = LocalPlayer.Character
            if myChar then
                local myHum = myChar:FindFirstChild("Humanoid")
                if myHum then
                    if not origHipHeight then origHipHeight = myHum.HipHeight end
                    myHum.WalkSpeed = 16
                    myHum.HipHeight = (myHum.RigType == Enum.HumanoidRigType.R6 and 0.5 or 1.2)
                end
            end
...
KillBtn.MouseButton1Click:Connect(function()
    ...
    if isKillMode then
        ...
        if targetPlr then
            ...
            -- تطبيق الارتفاع البسيط + السرعة 16 + تجميد المشي
            local myChar = LocalPlayer.Character
            if myChar then
                local myHum = myChar:FindFirstChild("Humanoid")
                if myHum then
        ...
        if (isSuicideMode or isKillMode) and targetName ~= "" then
            ...
            if myChar and targetPlr and targetPlr.Character then
                ...
                if myHum and myRoot and tHum and tRoot and myHum.Health > 0 and tHum.Health > 0 then
                    ...
                    -- التأكد المستمر من الارتفاع الخفيف والسرعة 16
                    if myHum.WalkSpeed ~= 16 then myHum.WalkSpeed = 16 end
                    local targetHip = (myHum.RigType == Enum.HumanoidRigType.R6 and 0.5 or 1.2)
                    if myHum.HipHeight ~= targetHip then myHum.HipHeight = targetHip end
