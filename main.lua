...
-- زر التتبع (يشتغل ويستمر لين ترجع تضغطه بنفسك)
...
-- نظام التتبع المستمر والمشي نحوه والانتحار عند الوصول (يبقى شغال لين توقفه أنت)
RunService.RenderStepped:Connect(function()
    if isFollowing and targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if myChar and myChar:FindFirstChild("Humanoid") and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            -- إذا وصل عنده (أقل من 4 أمتار) ولم يمت بعد
            if distance <= 4 and not hasKilled then
                hasKilled = true
                if myChar:FindFirstChildOfClass("Humanoid") then
                    myChar:FindFirstChildOfClass("Humanoid").Health = 0
                end
            else
                -- يكمل مشي نحوه باستمرار
