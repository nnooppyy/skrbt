-- ==========================================================
--               WAFI ULTIMATE HUB + HITBOX EXPANDER
-- ==========================================================
...
-- إعدادات الهيت بوكس المدمجة
local HitboxSettings = {
    Size = Vector3.new(5, 5, 5), -- الحجم المتوازن والآمن
    Transparency = 0.7,         -- درجة الشفافية (مربع شبه مخفي)
    CanCollide = false          -- إلغاء التصادم لعدم التعثر باللاعبين
}
...
-- ==========================================
-- بناء مكوّن القائمة المنسدلة (Player Dropdown Component)
-- ==========================================
...
-- إنشاء قائمة FarmPage وقائمة ViewPage
...
local function refreshSavedUI()
    ...
    local totalHeight = 0
    for i, id in ipairs(savedPlaceIds) do
        ...
        GoItemBtn.MouseButton1Click:Connect(function()
            TeleportService:Teleport(tonumber(id), LocalPlayer)
        end)
...
local function addSavedID(id)
    ...
    for _, v in ipairs(savedPlaceIds) do if v == numId then return end end
    table.insert(savedPlaceIds, numId)
    saveIDsToFile()
    refreshSavedUI()
end
...
CopyCurrentBtn.MouseButton1Click:Connect(function()
    ...
    copyToClipboard(tostring(currentId))
    addSavedID(currentId)
    CopyCurrentBtn.Text = "تم النسخ والحفظ بنجاح! ✓"
...
---------------------------------------------------------
-- محرك الاختفاء المحسن (Motor6D FE Desync Invisibility)
---------------------------------------------------------
...
-- اللوب التشغيلي الرئيسي (شامل تكبير الهيت بوكس والمهام الأخرى)
local lastMoveTick = 0
local wasMoving = false

getgenv().Wafi_MainLoop = task.spawn(function()
    while task.wait(0.01) do
        -- تشغيل الهيت بوكس المستمر لجميع اللاعبين الآخرين تلقائياً
