-- ==========================================================
--  WAFI ULTIMATE HUB (WITH SPEED SLIDER)
-- ==========================================================
...
local function hopToLowPlayerServer()
    ...
    pcall(function()
        local res
        if req then
            local raw = req({Url = url, Method = "GET"}).Body
            res = HttpService:JSONDecode(raw)
        else
            res = HttpService:JSONDecode(game:HttpGet(url))
        end
        ...
        if res and res.data then
            ...
            if #availableServers > 0 then
                local chosenServer = availableServers[1]
...
local Theme = {
    ...
    CardHover = Color3.fromRGB(32, 40, 60),
    Border = Color3.fromRGB(45, 65, 100),
    Accent = Color3.fromRGB(0, 140, 255),
    Text = Color3.fromRGB(245, 245, 247),
    SubText = Color3.fromRGB(130, 150, 185),
    ON = Color3.fromRGB(16, 185, 129),
    OFF = Color3.fromRGB(0, 140, 255)
}
...
local function stopMovement()
    ...
    if myChar then
        ...
        if myHum and myRoot then 
            ...
            if myRoot:FindFirstChild("Wafi_FloatVel") then
                myRoot.Wafi_FloatVel:Destroy()
            end
            if myRoot:FindFirstChild("Wafi_BodyPos") then
                myRoot.Wafi_BodyPos:Destroy()
            end
        end
    end
end
...
local function parsePlaceAndJob(text)
    ...
    local pId, jId = cleanText:match("(%d+),(%S+)")
    if pId and jId then
        return tonumber(pId), jId
    end
    local singleId = cleanText:match("(%d+)")
...
ToggleMenuBtn.MouseButton1Click:Connect(function()
    ...
    if MainFrame.Visible then
        ...
        TweenService:Create(MainFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 340, 0, 310)}):Play()
    end
end)
...
Sidebar.Size = UDim2.new(0, 85, 1, -38)
...
FarmPage.Size = UDim2.new(1, -85, 1, -38)
FarmPage.Position = UDim2.new(0, 85, 0, 38)
FarmPage.BackgroundTransparency = 1
FarmPage.Visible = true
FarmPage.Parent = MainFrame

local ViewPage = Instance.new("Frame")
ViewPage.Size = UDim2.new(1, -85, 1, -38)
ViewPage.Position = UDim2.new(0, 85, 0, 38)
...
MiscPage.Size = UDim2.new(1, -85, 1, -38)
MiscPage.Position = UDim2.new(0, 85, 0, 38)
...
ToggleBtn.Size = UDim2.new(0, 235, 0, 26)
...
AutoJumpBtn.Size = UDim2.new(0, 235, 0, 26)
AutoJumpBtn.Position = UDim2.new(0, 10, 0, 36)
AutoJumpBtn.BackgroundColor3 = Theme.Card
AutoJumpBtn.TextColor3 = Theme.OFF
AutoJumpBtn.Text = "النط التلقائي (كل 5 دقائق)  [ OFF ]"
AutoJumpBtn.TextSize = 10
AutoJumpBtn.Font = Enum.Font.GothamBold
AutoJumpBtn.Parent = FarmPage

local AutoJumpCorner = Instance.new("UICorner")
AutoJumpCorner.CornerRadius = UDim.new(0, 8)
AutoJumpCorner.Parent = AutoJumpBtn

local AutoJumpStroke = Instance.new("UIStroke")
AutoJumpStroke.Color = Theme.Border
AutoJumpStroke.Thickness = 1
AutoJumpStroke.Parent = AutoJumpBtn

local function createDropdown(parentFrame, pos, size, placeholder)
    ...
    DropBtn.Size = UDim2.new(0, 22, 1, 0)
    DropBtn.Position = UDim2.new(1, -22, 0, 0)
    DropBtn.BackgroundColor3 = Theme.Card
    ...
    Scroll.Position = UDim2.new(0, 0, 1, 4)
    Scroll.BackgroundColor3 = Theme.Sidebar
    ...
    return Box, DropBtn, Scroll
end

local PlayerBox, FarmDropBtn, FarmScroll = createDropdown(FarmPage, UDim2.new(0, 10, 0, 66), UDim2.new(0, 235, 0, 26), "اسم اللاعب أو اختره... ▼")
local ViewPlayerBox, ViewDropBtn, ViewScroll = createDropdown(ViewPage, UDim2.new(0, 10, 0, 8), UDim2.new(0, 235, 0, 28), "اسم اللاعب أو اختره... ▼")

-- ==========================================================
--               شريط السرعة التفاعلي (SPEED SLIDER)
-- ==========================================================
local moveSpeed = 60
local minSpeed = 5
local maxSpeed = 200
...
SpeedFrame.Size = UDim2.new(0, 235, 0, 34)
SpeedFrame.Position = UDim2.new(0, 10, 0, 96)
SpeedFrame.BackgroundColor3 = Theme.Card
...
SpeedLabel.Position = UDim2.new(0, 6, 0, 3)
SpeedLabel.BackgroundTransparency = 1
SpeedLabel.Text = "السرعة: " .. tostring(moveSpeed)
...
SliderBack.Size = UDim2.new(1, -12, 0, 8)
SliderBack.Position = UDim2.new(0, 6, 0, 20)
SliderBack.BackgroundColor3 = Theme.Sidebar
SliderBack.Parent = SpeedFrame
...
local initialPercent = (moveSpeed - minSpeed) / (maxSpeed - minSpeed)
...
local function updateSlider(input)
    ...
    moveSpeed = math.floor(minSpeed + (pct * (maxSpeed - minSpeed)))
...
local isOpenFarm = false
local isOpenView = false

local function refreshDropdownList(scrollObj, isFarm)
    for _, child in ipairs(scrollObj:GetChildren()) do
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            ...
            ItemBtn.Text = plr.DisplayName .. " (@" .. plr.Name .. ")"
            ItemBtn.TextSize = 10
            ItemBtn.Font = Enum.Font.Gotham
            ItemBtn.ZIndex = 16
            ItemBtn.Parent = scrollObj
            ...
            ItemBtn.MouseEnter:Connect(function()
                TweenService:Create(ItemBtn, TweenInfo.new(0.15), {BackgroundColor3 = Theme.Border}):Play()
            end)
            ItemBtn.MouseLeave:Connect(function()
                TweenService:Create(ItemBtn, TweenInfo.new(0.15), {BackgroundColor3 = Theme.Card}):Play()
            end)

            ItemBtn.MouseButton1Click:Connect(function()
                PlayerBox.Text = plr.Name
                ViewPlayerBox.Text = plr.Name
                targetName = plr.Name
                viewTargetName = plr.Name

                FarmScroll.Visible = false
                ViewScroll.Visible = false
                FarmDropBtn.Text = "▼"
                ViewDropBtn.Text = "▼"
                isOpenFarm = false
                isOpenView = false
    ...
    scrollObj.CanvasSize = UDim2.new(0, 0, 0, totalHeight)
end

FarmDropBtn.MouseButton1Click:Connect(function()
    isOpenFarm = not isOpenFarm
    if isOpenFarm then
        refreshDropdownList(FarmScroll, true)
        FarmScroll.Visible = true
        FarmDropBtn.Text = "▲"
    else
        FarmScroll.Visible = false
        FarmDropBtn.Text = "▼"
...
ViewDropBtn.MouseButton1Click:Connect(function()
    isOpenView = not isOpenView
    if isOpenView then
        refreshDropdownList(ViewScroll, false)
        ViewScroll.Visible = true
        ViewDropBtn.Text = "▲"
    else
        ViewScroll.Visible = false
        ViewDropBtn.Text = "▼"
...
GoBtn.Size = UDim2.new(0, 235, 0, 26)
GoBtn.Position = UDim2.new(0, 10, 0, 134)
...
KillBtn.Size = UDim2.new(0, 235, 0, 26)
KillBtn.Position = UDim2.new(0, 10, 0, 164)
KillBtn.BackgroundColor3 = Theme.Card
KillBtn.TextColor3 = Theme.OFF
KillBtn.Text = "الذهاب والذبح بالسكين  [ OFF ]"
KillBtn.TextSize = 11
...
InvisBtn.Size = UDim2.new(0, 235, 0, 26)
InvisBtn.Position = UDim2.new(0, 10, 0, 194)
...
BlueHighlightBtn.Size = UDim2.new(0, 235, 0, 28)
...
SpectateBtn.Size = UDim2.new(0, 235, 0, 28)
...
NameEspBtn.Size = UDim2.new(0, 235, 0, 28)
...
CopyCurrentBtn.Size = UDim2.new(0, 235, 0, 28)
...
JoinIdBtn.Size = UDim2.new(0, 235, 0, 28)
...
SavedHeader.Size = UDim2.new(0, 235, 0, 18)
...
SavedContainer.Size = UDim2.new(0, 235, 0, 0)
...
ServerHopBtn.Size = UDim2.new(0, 235, 0, 28)
ServerHopBtn.BackgroundColor3 = Theme.Card
ServerHopBtn.TextColor3 = Theme.OFF
ServerHopBtn.Text = "سيرفر بأقل عدد لاعبين"
...
RejoinBtn.Size = UDim2.new(0, 235, 0, 28)
...
applyHoverEffect(AutoJumpBtn)
applyHoverEffect(GoBtn)
applyHoverEffect(KillBtn)
applyHoverEffect(InvisBtn)
applyHoverEffect(BlueHighlightBtn)
applyHoverEffect(SpectateBtn)
applyHoverEffect(NameEspBtn)
applyHoverEffect(CopyCurrentBtn)
applyHoverEffect(JoinIdBtn)
applyHoverEffect(ServerHopBtn)
applyHoverEffect(RejoinBtn)
...
local function refreshSavedUI()
    ...
    local totalHeight = 0
    for i, idText in ipairs(savedPlaceIds) do
        ...
        local shortId = tostring(idText)
        if #shortId > 15 then shortId = shortId:sub(1, 15) .. "..." end
        IdLabel.Text = shortId
        ...
        GoItemBtn.MouseButton1Click:Connect(function()
            GoItemBtn.Text = "..."
            local pId, jId = parsePlaceAndJob(tostring(idText))
            pcall(function()
                if jId and jId ~= "" then
    ...
    SavedContainer.Size = UDim2.new(0, 235, 0, totalHeight)
end

local function addSavedID(id)
    local textId = tostring(id)
    if not textId or textId == "" then return end
    for _, v in ipairs(savedPlaceIds) do if v == textId then return end end
    table.insert(savedPlaceIds, textId)
    saveIDsToFile()
    refreshSavedUI()
end
...
CopyCurrentBtn.MouseButton1Click:Connect(function()
    ...
    local currentJobId = game.JobId
    local combinedId = currentPlaceId .. "," .. currentJobId
    
    copyToClipboard(combinedId)
    addSavedID(combinedId)
    
    CopyCurrentBtn.Text = "تم نسخ الماب والسيرفر معاً! ✓"
...
JoinIdBtn.MouseButton1Click:Connect(function()
    ...
    if pId then
        addSavedID(DirectIdBox.Text)
        JoinIdBtn.Text = "جاري النقل للسيرفر المخصص..."
        pcall(function()
            if jId and jId ~= "" then
                TeleportService:TeleportToPlaceInstance(pId, jId, LocalPlayer)
            else
...
ServerHopBtn.MouseButton1Click:Connect(function()
    ServerHopBtn.Text = "جاري البحث عن سيرفر قليل اللاعبين..."
    hopToLowPlayerServer()
end)

local isAFK = false
local isAutoJump = false
...
AutoJumpBtn.MouseButton1Click:Connect(function()
    ...
    AutoJumpBtn.Text = isAutoJump and "النط التلقائي (كل 5 دقائق)  [ ON ]" or "النط التلقائي (كل 5 دقائق)  [ OFF ]"
    AutoJumpBtn.TextColor3 = isAutoJump and Theme.ON or Theme.OFF
    AutoJumpStroke.Color = isAutoJump and Theme.ON or Theme.Border
end)
...
KillBtn.MouseButton1Click:Connect(function()
    ...
    if isKillMode then
        ...
        local targetPlr = findPlayer(targetName)
        if targetPlr then
            KillBtn.Text = "الذهاب والذبح بالسكين  [ ON ]"
...
-- ==========================================================
--        الحلقة الأساسية للانتقال والملاحقة والسرعة
-- ==========================================================
local lastJumpTick = os.clock()
local wasMoving = false
local heightOffset = 1.10
...
getgenv().Wafi_MainLoop = task.spawn(function()
    while task.wait(0.01) do
        pcall(function()
            ...
            -- النط التلقائي
            if isAutoJump and (os.clock() - lastJumpTick >= 300) then
                lastJumpTick = os.clock()
                if myChar then
                    local myHum = myChar:FindFirstChild("Humanoid")
            ...
            -- الذهاب والملاحقة والسرعة والذبح
            if (isSuicideMode or isKillMode) and targetName ~= "" then
                ...
                if myChar and targetPlr and targetPlr.Character then
                    ...
                    if myHum and myRoot and tHum and tRoot and myHum.Health > 0 and tHum.Health > 0 then
                        ...
                        local targetPos = Vector3.new(tRoot.Position.X, tRoot.Position.Y + heightOffset, tRoot.Position.Z)
                        local dir = (targetPos - myRoot.Position)
                        local dist = dir.Magnitude

                        if dist > 1.5 then
                            -- استخدام السرعة من السلايدر للتنقل
                            local step = math.min(dist, moveSpeed * 0.01)
                            myRoot.CFrame = CFrame.new(myRoot.Position + dir.Unit * step, targetPos)
                        else
