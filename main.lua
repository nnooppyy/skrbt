...
-- تطبيق السرعة والتتبع والتحكم الكامل بالموت اليدوي
RunService.RenderStepped:Connect(function()
    local myChar = LocalPlayer.Character
    if myChar and myChar:FindFirstChild("Humanoid") then
        myChar.Humanoid.WalkSpeed = currentSpeed
        
        -- التتبع والمشي نحو اللاعب (يعمل فقط إذا كان الزر ON وتم تفعيل التتبع)
            ...
            if targetPlr and targetPlr.Character and targetPlr.Character:FindFirstChild("HumanoidRootPart") then
                if myChar.Humanoid.Health > 0 then
                    ...
                    if distance <= 5 and not hasKilledCurrentLife then
                        ...
                        myChar.Humanoid.Health = 0 -- يذبح نفسه فقط إذا وصل له أثناء تشغيل الزر
