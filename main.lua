...
local hasMovedAway = false -- التأكد أنه تحرك فعلياً وابتعد قبل السماح بالموت
...
-- تنفيذ الركض والموت فقط عند الوصول الفعلي للهدف بعد تشغيل الخيار
        ...
        if isFollowing and targetName ~= "" and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            if targetPlr and targetPlr.Character and targetPlr.Character:FindFirstChild("HumanoidRootPart") then
                if myChar.Humanoid.Health > 0 then
                    ...
                    -- إذا كانت المسافة أكثر من 7 أمتار، نعتبر أنه ابتعد عن الهدف (يعني بدأ الرحلة)
                    if distance > 7 then
                        hasMovedAway = true
                    end
                    
                    -- لن يذبح نفسه إلا إذا كان الزر ON + وابتعد ثم عاد ووصل لمسافة أقل من 4 أمتار عنده
                    if distance <= 4 and hasMovedAway and not hasKilledCurrentLife then
