...
-- إنشاء الواجهة الرئيسية (بدون شريط السرعة)
...
-- 3. زر الذهاب والانتحار (انتحار فوري عند الوصول)
...
-- حماية الرسبن الخفيفة جداً (0.2 ثانية فقط لنقل موقعك للرسبن)
...
-- اللوب التشغيلي السريع (ينفذ كل 0.02 ثانية للاستجابة اللحظية)
        ...
        if targetName ~= "" then
            ...
            if myChar and targetPlr and targetPlr.Character then
                ...
                if myHum and myRoot and tHum and tRoot and myHum.Health > 0 and tHum.Health > 0 then
                    ...
                    -- الانتحار اللحظي المباشر أول ما يقرب منه
                    if isSuicideMode then
                        if dist <= 4 and not justSpawned then
                            myHum.Health = 0 -- يذبح نفسه في نفس اللحظة
                        else
