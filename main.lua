-- ==========================================================
--        WAFI ULTIMATE HUB (OFF-GROUND KILL 1.10)
-- ==========================================================
...
ToggleMenuBtn.MouseButton1Click:Connect(function()
    ...
    if MainFrame.Visible then
        ...
        TweenService:Create(MainFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 340, 0, 275)}):Play()
    end
end)
...
GoBtn.Position = UDim2.new(0, 10, 0, 96)
...
KillBtn.Position = UDim2.new(0, 10, 0, 126)
...
InvisBtn.Position = UDim2.new(0, 10, 0, 156)
...
-- ==========================================================
--                   الحلقة الأساسية للعمليات
-- ==========================================================
local lastMoveTick = 0
local lastJumpTick = os.clock()
local wasMoving = false
local heightOffset = 1.10 -- تم تقليل الارتفاع عن سطح الأرض إلى 1.10
...
getgenv().Wafi_MainLoop = task.spawn(function()
    while task.wait(0.01) do
        pcall(function()
            ...
            -- النط التلقائي الخاص بالزر (كل 5 دقائق = 300 ثانية)
            ...
            -- الملاحقة والذبح مع الارتفاع عن الأرض بمقدار 1.10
            if (isSuicideMode or isKillMode) and targetName ~= "" then
                ...
                if myChar and targetPlr and targetPlr.Character then
                    ...
                    if myHum and myRoot and tHum and tRoot and myHum.Health > 0 and tHum.Health > 0 then
                        ...
                        -- قوة تثبيت لمنع السقوط مع الجاذبية أثناء الطيران السريع
                        local floatVel = myRoot:FindFirstChild("Wafi_FloatVel")
                        if not floatVel then
                            floatVel = Instance.new("BodyVelocity")
                            floatVel.Name = "Wafi_FloatVel"
                            floatVel.MaxForce = Vector3.new(0, 100000, 0)
                            floatVel.Velocity = Vector3.new(0, 0, 0)
                            floatVel.Parent = myRoot
                        end

                        -- حساب الموضع المرتفع بمقدار 1.10
                        local elevatedTargetPos = Vector3.new(tRoot.Position.X, tRoot.Position.Y + heightOffset, tRoot.Position.Z)
                        local dist = (Vector3.new(myRoot.Position.X, 0, myRoot.Position.Z) - Vector3.new(tRoot.Position.X, 0, tRoot.Position.Z)).Magnitude
                        
                        if os.clock() - lastMoveTick >= 0.05 then
                            myHum:MoveTo(elevatedTargetPos)
                            myRoot.CFrame = CFrame.new(Vector3.new(myRoot.Position.X, tRoot.Position.Y + heightOffset, myRoot.Position.Z), elevatedTargetPos)
