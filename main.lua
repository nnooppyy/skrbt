-- ==========================================================
--          WAFI ULTIMATE HUB + ENHANCED REACH HITBOX
-- ==========================================================
...
-- إعدادات الهيت بوكس والـ Reach المحسّنة
local HitboxSettings = {
    Size = Vector3.new(12, 12, 12), -- حجم الهيت بوكس المحسّن
    Transparency = 0.6,
    CanCollide = false
}
    ...
    if readfile and isfile and isfile(AutoHopFileName) then
        ...
        if isSearching then
            ...
            if found then
                ...
                warn("[WAFI HUB] تم العثور على شرطي غني: " .. copPlayer.Name)
            else
                warn("[WAFI HUB] جاري البحث في سيرفر عشوائي...")
                hopToRandomServer()
            end
        end
    end
...
ServerHopBtn.Text = "بحث (سيرفر عشوائي) +500k"
...
ServerHopBtn.MouseButton1Click:Connect(function()
    ...
    if isSearching then
        ...
        if found then
            ...
            ServerHopBtn.Text = "جاري النقل لسيرفر عشوائي..."
            hopToRandomServer()
        end
    end
end)
...
-- ==========================================================
-- نظام الهيت بوكس المحسّن والتأثير التلقائي باللمس (Touch/Reach)
-- ==========================================================
getgenv().Wafi_MainLoop = task.spawn(function()
    while task.wait(0.01) do
        pcall(function()
            local myChar = LocalPlayer.Character
            local knife = myChar and getKnifeTool(myChar)
                    ...
                    if hrp and humanoid and humanoid.Health > 0 then
                        ...
                        if knife and knife.Parent == myChar then
                            ...
                            if handle then
                                ...
                                -- إذا كان الخصم داخل مدى الهيت بوكس المكبّر
                                if dist <= (HitboxSettings.Size.X + 4) then
            ...
            if (isSuicideMode or isKillMode) and targetName ~= "" then
                ...
                if myChar and targetPlr and targetPlr.Character then
                    ...
                    if myHum and myRoot and tHum and tRoot and myHum.Health > 0 and tHum.Health > 0 then
                        ...
                        if isSuicideMode then
                            if dist <= (HitboxSettings.Size.X / 2 + 2) then
                                myHum.Health = 0
                            end
                        elseif isKillMode then
                            if dist <= (HitboxSettings.Size.X + 2) then
