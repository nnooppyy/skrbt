-- ==========================================================
--          WAFI ULTIMATE HUB + HITBOX EXPANDER [FIXED]
-- ==========================================================
...
-- ==========================================================
-- نظام الفحص والبحث التلقائي عن العساكر
-- ==========================================================
...
local function hasCopsInServer()
    ...
    pcall(function()
        ...
        if res and res.data then
            ...
            if #candidateServers > 0 then
                local chosenServer = candidateServers[math.random(1, math.min(#candidateServers, 10))]
...
-- فحص تلقائي فور دخول السيرفر في حال كان وضع البحث مفعلاً
task.spawn(function()
    if readfile and isfile and isfile(AutoHopFileName) then
        local isSearching = false
        pcall(function() isSearching = (readfile(AutoHopFileName) == "true") end)
        if isSearching then
            task.wait(3) -- انتظار تحميل الفرق واللاعبين
            if hasCopsInServer() then
                if writefile then pcall(function() writefile(AutoHopFileName, "false") end) end
                warn("[WAFI HUB] تم العثور على سيرفر يحتوي على عساكر!")
            else
                warn("[WAFI HUB] لا يوجد عساكر، جاري التنقل لسيرفر آخر...")
                hopToNextServer()
            end
        end
    end
...
ServerHopBtn.Text = "بحث عن سيرفر فيه عساكر"
...
ServerHopBtn.MouseButton1Click:Connect(function()
    ...
    if isSearching then
        ...
        ServerHopBtn.Text = "جاري فحص السيرفر الحالي..."
        if hasCopsInServer() then
            ServerHopBtn.Text = "السيرفر الحالي فيه عساكر!"
            task.wait(1.5)
            ServerHopBtn.Text = "بحث عن سيرفر فيه عساكر"
        else
            ServerHopBtn.Text = "جاري التنقل لبحث عن عساكر..."
            hopToNextServer()
        end
    end
end)
